...
namespace HB.FullStack.WebApi.Security
{
    public class DefaultSecurityService : ISecurityService
    {
        ...
        public async Task<byte[]> ProcessFormFileAsync(IFormFile? formFile, string[] permittedFileSuffixes, long sizeLimit)
        {
            ...
            {
                throw WebApiExceptions.UploadError(cause: "empty file.", formFile?.FileName, null);
            }

            if (formFile.Length > sizeLimit)
            {
                throw WebApiExceptions.UploadError(cause: "Upload OverSize", formFile.FileName, null);
            }
            ...
            try
            {
                ...
                {
                    throw WebApiExceptions.UploadError(cause: "empty file after removing BOM.", formFile.FileName, null);
                }

                if (!IsValidFileExtensionAndSignature(formFile.FileName, memoryStream, permittedFileSuffixes))
                {
                    throw WebApiExceptions.UploadError(cause: "Upload Wrong Type Files", formFile.FileName, null);
                }
                else
                {
            ...
            {
                throw WebApiExceptions.UploadError(cause: "Unkown file upload error", formFile.FileName, innerException: ex);
            }
        }
