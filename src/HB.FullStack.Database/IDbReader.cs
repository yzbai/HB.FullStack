...
namespace HB.FullStack.Database
{
    public interface IDbReader
    {
        ...
        Task<long> CountAsync<T>(Expression<Func<T, bool>> whereExpr, TransactionContext? transContext) where T : BaseDbModel, new();

        Task<long> CountAsync<T>(FromExpression<T>? fromCondition, WhereExpression<T>? whereCondition, TransactionContext? transContext) where T : BaseDbModel, new();

        Task<long> CountAsync<T>(TransactionContext? transContext) where T : BaseDbModel, new();
        ...
        Task<T?> ScalarAsync<T>(Expression<Func<T, bool>> whereExpr, TransactionContext? transContext) where T : BaseDbModel, new();

        Task<T?> ScalarAsync<T>(FromExpression<T>? fromCondition, WhereExpression<T>? whereCondition, TransactionContext? transContext) where T : BaseDbModel, new();

        Task<T?> ScalarAsync<T>(long id, TransactionContext? transContext) where T : DbModel2<long>, new();

        Task<T?> ScalarAsync<T>(Guid id, TransactionContext? transContext) where T : DbModel2<Guid>, new();

        Task<T?> ScalarAsync<T>(WhereExpression<T>? whereCondition, TransactionContext? transContext) where T : BaseDbModel, new();
