...
namespace HB.FullStack.Database
{
    ...
    partial class DefaultDatabase
    {
            ...
            long? newTimestamp = null) where T : TimestampDbModel, new()
        {
            ...
            if (timestamp <= 0)
            {
                throw DbExceptions.TimestampShouldBePositive(timestamp);
            }
            ...
            try
            {
                DbEngineCommand command = DbCommandBuilder.CreateUpdatePropertiesCommand(
                    modelDef: modelDef,
                    id: id,
                ...
                {
                    throw DbExceptions.ConcurrencyConflict(modelDef.ModelFullName, $"使用Timestamp版本的乐观锁，出现冲突。id:{id}, lastUser:{lastUser}, timestamp:{timestamp}, propertyValues:{SerializeUtil.ToJson(propertyNameValues)}", "");
                }
                else
                {
                    throw DbExceptions.FoundTooMuch(modelDef.ModelFullName, $"id:{id}, timestamp:{timestamp}, propertyValues:{SerializeUtil.ToJson(propertyNameValues)}");
                }
            }
            catch (Exception ex) when (ex is not DbException)
            {
                throw DbExceptions.UnKown(modelDef.ModelFullName, $"id:{id}, timestamp:{timestamp}, propertyValues:{SerializeUtil.ToJson(propertyNameValues)}", ex);
            }
        }
            ...
            long? newTimestamp = null) where T : DbModel, new()
        {
            ...
            try
            {
                DbEngineCommand command = DbCommandBuilder.CreateUpdatePropertiesUsingOldNewCompareCommand(
                    modelDef,
                    id,
                ...
                {
                    throw DbExceptions.ConcurrencyConflict(modelDef.ModelFullName, $"使用新旧值对比的乐观锁出现冲突。id:{id}, lastUser:{lastUser}, propertyOldNewValues:{SerializeUtil.ToJson(propertyNameOldNewValues)}", "");
                }
                else
                {
                    throw DbExceptions.FoundTooMuch(modelDef.ModelFullName, $"id:{id}, lastUser:{lastUser}, propertyOldNewValues:{SerializeUtil.ToJson(propertyNameOldNewValues)}");
                }
            }
            catch (Exception ex) when (ex is not DbException)
            {
                throw DbExceptions.UnKown(modelDef.ModelFullName, $"id:{id}, lastUser:{lastUser}, propertyOldNewValues:{SerializeUtil.ToJson(propertyNameOldNewValues)}", ex);
            }
        }
            ...
            TransactionContext? transactionContext) where T : TimestampDbModel, new()
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (matched != 1)
                    {
                        throw DbExceptions.ConcurrencyConflict(modelDef.ModelFullName, SerializeUtil.ToJson(modelChanges), "BatchUpdatePropertiesAsync");
                    }
            ...
            {
                throw DbExceptions.UnKown(modelDef.ModelFullName, SerializeUtil.ToJson(modelChanges), ex);
            }
        }
            ...
            TransactionContext? transactionContext = null) where T : DbModel, new()
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (matched != 1)
                    {
                        throw DbExceptions.ConcurrencyConflict(modelDef.ModelFullName, SerializeUtil.ToJson(updateChanges), "BatchUpdatePropertiesAsync");
                    }
            ...
            {
                throw DbExceptions.UnKown(modelDef.ModelFullName, SerializeUtil.ToJson(updateChanges), ex);
            }
        }
        ...
        private static List<(string propertyName, object? oldValue, object? newValue)> ConvertChangedPackToList(ChangedPack changedPropertyPack, DbModelDef modelDef, out long? newTimestamp)
        {
            ...
            {
                throw DbExceptions.ChangedPropertyPackError("ChangedProperties为空或者Id为null", changedPropertyPack, modelDef.ModelFullName);
            }
            ...
            IList<(object id, IList<(string propertyName, object? propertyValue)> properties, long oldTimestamp, long? newTimestamp)> modelChanges)
        {
            ...
            foreach ((object id, IList<(string propertyName, object? propertyValue)> properties, long oldTimestamp, long? newTimestamp) in modelChanges)
            {
                ...
                {
                    throw DbExceptions.TimestampShouldBePositive(oldTimestamp);
                }
            ...
            IList<(object id, IList<(string propertyName, object? oldPropertyValue, object? newPropertyValue)> properties, long? newTimestamp)> modelChanges)
        {
            ...
            foreach ((object id, IList<(string propertyNames, object? oldPropertyValues, object? newPropertyValues)> properties, long? newTimestamp) in modelChanges)
            {
                ...
                {
                    throw DbExceptions.TimestampShouldBePositive(newTimestamp.Value);
                }
