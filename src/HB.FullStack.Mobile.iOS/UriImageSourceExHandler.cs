...
{
	public sealed class UriImageSourceExHandler : IImageSourceHandler, IAnimationSourceHandler
	{
		[Foundation.Preserve(Conditional = true)]
		public UriImageSourceExHandler()
		{
		}
		...
		public async Task<UIImage> LoadImageAsync(ImageSource imagesource, CancellationToken cancelationToken = default(CancellationToken), float scale = 1f)
		{
			...
			if (image == null)
			{
				Log.Warning(nameof(UriImageSourceExHandler), "Could not load image: {0}", imageLoader);
			}
