...
namespace HB.Framework.Cache
{
    public static class IDistributedCacheExtensions
    {
        ...
        public static async Task<(int, bool)> GetIntAsync(this IDistributedCache cache, string key, CancellationToken token = default(CancellationToken))
        {
            try
            {
                (string? value, bool exists) = await cache.GetStringAsync(key, token).ConfigureAwait(false);
        ...
        public static async Task<(string?, bool)> GetStringAsync(this IDistributedCache cache, string key, CancellationToken token = default(CancellationToken))
        {
        ...
        //public static async Task<bool> IsExistThenRemoveAsync(this IDistributedCache cache, string key, CancellationToken token = default(CancellationToken))
        //{
        //    IDatabase database = await cache.GetDefaultDatabaseAsync().ConfigureAwait(false);

        //    token.ThrowIfCancellationRequested();

        //    return await database.KeyDeleteAsync(cache.GetRealKey(key)).ConfigureAwait(false);
        //}
    }
}
